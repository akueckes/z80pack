...
void config(void)
{
	...
	if ((fp = fopen(fn, "r")) != NULL) {
		while (fgets(buf, BUFSIZE, fp) != NULL) {
			...
			if (!strcmp(t1, "sio1a_upper_case")) {
				...
				*s = 0;
				...
				for (i=0; i<64; i++) {
					t2 = strtok(NULL, " \t,");
					if (t2 == NULL) break;
					d7a_axis_map[j][i] = strtol(t2, NULL, 0);
					if ((d7a_axis_map[j][i] < 0) || (d7a_axis_map[j][i] > 255)) {
						LOGW(TAG, "invalid value for %s: %s\n", t1, t2);
						break;
					}
					d7a_do_map_axis[j] = true;
				}
				d7a_axis_map[j][i] = -1;
				continue;
				...
				for (i=0; i<64; i++) {
					t2 = strtok(NULL, " \t,");
					if (t2 == NULL) break;
					d7a_button_map[j][i] = strtol(t2, NULL, 0);
					if ((d7a_button_map[j][i] < 0) || (d7a_button_map[j][i] > 65535)) {
						LOGW(TAG, "invalid value for %s: %s", t1, t2);
						break;
					}
					d7a_do_map_buttons[j] = true;
				}
				d7a_button_map[j][i] = -1;
				continue;
#endif
#ifdef HAS_MUSICBOARD
			} else if (!strcmp(t1, "musicboard_sample_rate")) {
				musicboard_sample_rate = strtol(t2, NULL, 0);
			} else if (!strcmp(t1, "musicboard_buffer_size")) {
				musicboard_buffer_size = strtol(t2, NULL, 0);
			} else if (!strcmp(t1, "musicboard_sync_adjust")) {
				musicboard_sync_adjust = atof(t2);
			} else if (!strcmp(t1, "musicboard_recording_limit")) {
				musicboard_recording_limit = strtol(t2, NULL, 0);
			} else if (!strcmp(t1, "musicboard_soundfile")) {
				musicboard_soundfile = strdup(t2);
				for (s=musicboard_soundfile; *s > 31 && *s <127; s++);
				*s = 0;
			} else if (!strcmp(t1, "musicboard_stats")) {
				switch (*t2) {
				case '0':
					musicboard_stats = false;
					break;
				case '1':
					musicboard_stats = true;
					break;
				default:
					LOGW(TAG, "invalid value for %s: %s", t1, t2);
					break;
			...
			} else if (!strcmp(t1, "noisemaker_buffer_size")) {
				noisemaker_buffer_size = strtol(t2, NULL, 0);
			} else if (!strcmp(t1, "noisemaker_sync_adjust")) {
				noisemaker_sync_adjust = atof(t2);
				...
				*s = 0;
				...
				else if (!strncmp(t2, "gradient", 8)) vector_graphic_hires_mode = 1;
				...
				switch (*t2) {
				case '0':
					tarbell_rom_enabled = false;
					break;
				case '1':
					tarbell_rom_enabled = true;
					break;
				default:
					LOGW(TAG, "invalid value for %s: %s", t1, t2);
					break;
